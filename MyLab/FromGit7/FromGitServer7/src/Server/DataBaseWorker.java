...
public class DataBaseWorker {
 ...
 /*  private static synchronized ArrayList<Dragon> rewriteCollecttion(ArrayList<Dragon> dragons, int id) throws SQLException{
        Statement statement = connection.createStatement();
        ResultSet resultSet = statement.executeQuery("SELECT*FROM dragons");
        dragons.clear();
        while (resultSet.next()){
        String jsonDragon = resultSet.getString(1);
        JsonWorker jsonWorker = new JsonWorker();
        dragons.add(jsonWorker.transformString(jsonDragon));
        statement = connection.createStatement();
        statement.executeQuery("INSERT INTO dragons where id = '"+id+'")
        }
        return dragons;
    }*/
